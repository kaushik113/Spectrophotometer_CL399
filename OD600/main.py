...
# Function to display absorbance of red color
def display_absorbance(absorbance):
    tft.clear(tft.BLACK)
    tft.font(tft.FONT_DejaVu24)
    tft.text(10, 50, "Absorbance (Red):", tft.WHITE)
    tft.text(10, 80, "{:.2f}".format(absorbance), tft.RED)
...
try:
    while True:
        if btnA.wasPressed():  # Check if the button is pressed
            ...
            display_absorbance(absorbance_red)
